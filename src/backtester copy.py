...
class Backtester:
    ...
    def run_backtest(self):
        ...
        return performance_metrics, new_table_rows
...
if __name__ == "__main__":
    ...
    performance_metrics, new_table_rows = backtester.run_backtest()
    performance_df = backtester.analyze_performance()

    df = pd.DataFrame(new_table_rows, columns =['Date', 'Ticker', 'Action', 'Quantity', 'Price', 'Shares', 'Position Value',
                                      'Bullish', 'Bearish', 'Neutral'])
    df.to_csv(tickers[0] + '_' + selected_analysts[0] + '.csv', index=False)  
    performance_df.to_csv(tickers[0] + '_' + selected_analysts[0] + '_performance_df.csv', index=False)  